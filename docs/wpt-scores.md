...
| 8 | **iframe realm emulation: per-frame globals are leaky** — the single biggest remaining cluster (`Range-surroundContents` 690/1840, plus `Range-extractContents`/`cloneContents`/`deleteContents`, all on the shared `dom/common.js` harness). **Root cause fully traced** (`src/js.c`). Nordstjernen runs every frame in **one shared QuickJS realm**; `ns_js_run_iframe_scripts` concatenates a frame's classic scripts and wraps them as `(function(window,self,globalThis,top,parent,document,location,history){ with(window){ …scripts… } /*expose*/ })`, called with a per-frame `window` Proxy (`ns_iframe_scope_bootstrap`). Top-level `var`/`function` therefore land in the **wrapper function scope**, not on the frame `window`; a post-pass (`ns_js_iframe_collect_exposed_names` + `ns_js_iframe_append_exposed_globals`) re-publishes detected names onto the frame `window` as live `eval`-based accessor properties, guarded by `if (k in window) return;`. This emulation is consistent for a single load, but the WPT range harness reuses one `<iframe>` and, per `(range,node)` cell, **replaces `documentElement` and re-runs `setupRangeTests`**; the `with(window)` lookup, the wrapper-scope closure variable, the `if (k in window)` guard and the accessor's own `eval(k)` scope then disagree about which `paras` binding is current, so `run()`'s `eval("[paras[0]…]")` and the parent's `iframe.contentWindow.paras` read a stale/disconnected `paras` while `setupRangeTests`'s own view is correct → `paras[0].firstChild` null → range setup throws `TypeError`. Minimal repro: external `<script> var x=123` in an iframe → inside the frame `x===123` but `window.x===undefined`/`hasOwn` false (top page: both 123, hasOwn true). **Attempted** swapping the fallback `ns_js_eval` wrapper for a `global.document` swap — correct for the fallback but it does not touch the real `ns_js_run_iframe_scripts` path and would leak `var`s across frames on the shared global, so reverted. **Proper fix = give each iframe its own global object/realm** (so `var x` → that frame's global = that frame's `window`, shared consistently by all its scripts), retiring the concat+`with`+exposed-accessor emulation. Large, high-risk (this scaffolding is what makes the 690 pass) — needs design + broad re-test. | `Range-surroundContents.html` (690/1840); repro: iframe external-script `var x` vs `window.x` | ~1150 (blocked, root-caused) |
