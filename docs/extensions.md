...
- **Manifest** parsing: `name`, `version`, the extension id from
  ...
  falling back to the directory name), and `content_scripts`.
...
host, no `tabs`/`windows`/`webRequest`/`declarativeNetRequest`, no
toolbar/popup UI, and `getURL()` returns a `file://` URL (there is no
`moz-extension://` scheme). Extensions are unsigned and fully trusted —
