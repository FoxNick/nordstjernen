...
the WebGL API more or less directly onto **OpenGL ES** through GTK's
`GdkGLContext` and **libepoxy** — the same GL stack GTK 4 already uses to
composite the window. There is no ANGLE layer and no GL command-stream
validator between the page and the driver: WebGL calls become GLES calls.
...
most once. If there is no display (headless / `--dump` / `--screenshot`
runs with no GPU), `getContext` simply returns `null`.
...
1. **Context** — `gdk_display_create_gl_context()` creates a surfaceless
   `GdkGLContext`; `gdk_gl_context_set_allowed_apis(GDK_GL_API_GLES)` asks
   for OpenGL ES so that GLSL ES shaders (`#version 100` / `#version 300
   es`) compile natively, no translation.
...
./builddir/src/nordstjernen path/to/your-webgl-page.html
```
