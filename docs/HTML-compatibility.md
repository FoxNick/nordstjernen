...
|-----|:--:|------|
| `MessageEvent` | ✅ | |
| `WebSocket` (`ws://`/`wss://`) | ✅ | libcurl's native WebSocket API (`curl_ws_send`/`curl_ws_recv`: text/binary, fragmentation, ping/pong, close, subprotocol negotiation). Requires a libcurl built **with WebSocket support** — stock distro builds enable it from curl ≈ 8.11 (Ubuntu 25.04+, Debian 13); on older distro curls (Ubuntu 24.04, Debian 12) the protocol is compiled out and the `WebSocket` constructor throws |
