...
| `video` | 🟡 | poster + play overlay; click hands the source URL to the system media player (`ns_media_try_launch`) — no in-process codec. Streaming `<video>` (MSE/`blob:`, no file URL) hands the *page* URL instead, resolved by mpv/VLC + yt-dlp. **Navigating to a recognised video-page URL** (YouTube `watch`/`shorts`/`embed`/`live`, `youtu.be`, `music.youtube.com` — `ns_media_is_video_page` in `src/media.c`) hands off to the external player automatically *before* loading the page, so a single-codec browser plays the video without rendering (or crashing on) the site's heavy player app; if no player is installed it falls back to loading the page |
...
| `canvas` WebGPU context | 🟡 | experimental, off by default. In the `-Dwebgpu=enabled` build over wgpu-native (gated by `NS_WEBGPU_ALLOW`), `navigator.gpu` + `getContext("webgpu")` support a near-complete **render** path: WGSL shaders (naga), bind groups / uniforms / samplers / textures, render & depth pipelines, vertex/index buffers, MSAA, and the texture-to-canvas output — enough that **three.js's `WebGPURenderer` renders on it** (no WebGL2 fallback). Compute pipelines, image-to-texture upload, and query sets are not done yet. See [`docs/webgpu.md`](webgpu.md) |
