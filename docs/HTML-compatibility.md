...
Snapshot: **1.0.10**, 2026-06-17 (rev 7).
...
| Topic | Status | Notes |
|-------|:--:|------|
| `hidden` attribute | 🟡 | `hidden` maps to `display:none`; `HTMLElement.hidden` follows the spec's enumerated getter/setter for `"until-found"`; fragment/hash navigation runs the ancestor reveal path before scrolling, removing `hidden="until-found"` ancestors and opening skipped `<details>` ancestors, with `beforematch` fired for same-document hidden-until-found reveals. The remaining approximation is that hidden-until-found is not rendered with `content-visibility:hidden` before reveal |
...
| Event dispatch / cancellation | 🟡 | DOM listeners, bubbling, `preventDefault()`, and inline `return false`; cancellation now honours `event.cancelable`. A primary activation fires the full UI-Events button sequence `pointerdown`→`mousedown`→`pointerup`→`mouseup`→`click` (`ns_browser_click` in `src/libnordstjernen.c`) |
...
| `spellcheck` | ✅ | the enumerated content attribute and the boolean `HTMLElement.spellcheck` IDL attribute are spec-correct (`ns_node_spellcheck_used` in `src/js.c`): `true`/`""` → `true`, `false` → `false`, and the default/invalid state inherits the nearest ancestor's value, falling back to `true` at the root; the setter writes `"true"`/`"false"` while `getAttribute` keeps the literal. Whether to *visually* check spelling is explicitly UA-defined by the spec, and (like the lack of a bundled dictionary) Nordstjernen performs no on-screen spell-checking — the conformant surface is the attribute model, which is complete |
