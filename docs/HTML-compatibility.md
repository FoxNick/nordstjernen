...
|-----|:--:|------|
| `MessageEvent` | ✅ | |
| `WebSocket` (`ws://`/`wss://`) | ✅ | libcurl's native WebSocket API (`curl_ws_send`/`curl_ws_recv`: text/binary, fragmentation, ping/pong, close, subprotocol negotiation). The build requires libcurl ≥ 8.11, where distro builds ship WebSocket enabled (Ubuntu 25.04+, Debian 13); should the runtime libcurl still lack the `ws`/`wss` protocols, the `WebSocket` constructor throws instead of failing silently |
