...
| `canvas` WebGPU context | 🟡 | experimental, off by default. In the `-Dwebgpu=enabled` build over wgpu-native (gated by `NS_WEBGPU_ALLOW`), `navigator.gpu` and `getContext("webgpu")` work for device/buffer use and a **full draw path**: WGSL shader modules (naga), render pipelines, vertex/index buffers, render passes, `setPipeline`/`draw`, composited to the canvas — a vertex-coloured triangle renders. Bind groups / uniforms, samplers, depth/stencil, and compute pipelines are not bound yet. See [`docs/webgpu.md`](webgpu.md) |
