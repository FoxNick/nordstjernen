...
| `canvas` WebGPU context | 🟡 | experimental, off by default. In the `-Dwebgpu=enabled` build over wgpu-native (gated by `NS_WEBGPU_ALLOW`), `navigator.gpu` and `getContext("webgpu")` work for device/buffer use and a **render-pass clear path** (configure → getCurrentTexture → render pass → submit, composited back to the canvas). Geometry pipelines/shaders/bind groups are not bound yet. See [`docs/webgpu.md`](webgpu.md) |
