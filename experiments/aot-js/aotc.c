...
int main(int argc, char **argv) {
    ...
    int registry_mode = (entry && strcmp(entry, "--registry") == 0);
    if (registry_mode) entry = NULL;
    ...
    if (!registry_mode) {
        ...
        int any = 0;
        for (int i = 0; i < g_nfns; i++) if (g_fns[i].eligible) any = 1;
        if (!any) { fprintf(stderr, "aotc: no AOT-eligible functions\n"); return DECLINE; }
    }
    ...
    fprintf(o, "#include <stdio.h>\n#include <stdlib.h>\n#include <stdint.h>\n"
               "#include <math.h>\n#include <time.h>\n");
    if (registry_mode) fprintf(o, "#define NS_AOT_DISPATCH 1\n#include \"quickjs.h\"\n");
    fprintf(o, "\n");
    fprintf(o,
    ...
    if (registry_mode) {
        ...
        for (int i = 0; i < g_nfns; i++) {
            Fn *F = &g_fns[i];
            if (!F->eligible || F->has_array_args) continue;
            int ac = F->b->arg_count;
            if (ac > 8 || !valid_js_name(F->name)) continue;
            fprintf(o, "static double %s_aot(const double *a) { (void)a; return %s(",
                    F->mangled, F->mangled);
            for (int j = 0; j < ac; j++) fprintf(o, "%sa[%d]", j ? ", " : "", j);
            fprintf(o, "); }\n");
            fprintf(o, "static const JSAOTEntry %s_entry = { %s_aot, %d };\n",
                    F->mangled, F->mangled, ac);
            nreg++;
        }
        fprintf(o, "\nstatic const struct { const char *name; const JSAOTEntry *e; }\n"
                   "  ns_aot_table[] = {\n");
        for (int i = 0; i < g_nfns; i++) {
            Fn *F = &g_fns[i];
            if (!F->eligible || F->has_array_args || F->b->arg_count > 8 || !valid_js_name(F->name))
                continue;
            fprintf(o, "    { \"%s\", &%s_entry },\n", F->name, F->mangled);
        }
        fprintf(o, "  };\n\n");
        fprintf(o,
            "int ns_aot_register(JSContext *ctx, JSValueConst obj) {\n"
            "  int n = 0;\n"
            "  for (unsigned i = 0; i < sizeof(ns_aot_table)/sizeof(ns_aot_table[0]); i++) {\n"
            "    JSValue f = JS_GetPropertyStr(ctx, obj, ns_aot_table[i].name);\n"
            "    if (JS_IsFunction(ctx, f)) { JS_SetFunctionAOT(ctx, f, ns_aot_table[i].e); n++; }\n"
            "    JS_FreeValue(ctx, f);\n"
            "  }\n"
            "  return n;\n"
            "}\n");
        fclose(o);
        fprintf(stderr, "aotc: registry of %d AOT function(s) -> %s\n", nreg, out);
        return 0;
    }
