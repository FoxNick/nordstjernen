...
texture_surface_cached(ns_texture *tex, const char *filter_kw)
{
    ...
    GQuark q = ns_texture_surface_quark();
...
paint_walk(cairo_t *cr, const ns_box *b, const char *highlight)
{
    ...
    const ns_css_value *tv = style ? style->values[NS_CSS_TRANSFORM] : NULL;
    ...
    if (has_transform) {
        ...
        const ns_css_value *origin =
            style ? style->values[NS_CSS_TRANSFORM_ORIGIN] : NULL;
        ...
        const ns_css_transform *tf = anim_tf ? anim_tf : &tv->u.transform;
        for (int i = 0; i < tf->n_ops; i++) {
            const ns_css_transform_op *op2 = &tf->ops[i];
            switch (op2->kind) {
    ...
    if (!box_offscreen) {
        ...
        if (b->kind == NS_BOX_INLINE) paint_inline(cr, b, highlight);
        if (b->kind == NS_BOX_IMAGE)  paint_image(cr, b);
        if (b->kind == NS_BOX_VIDEO)  paint_video(cr, b);
    }
