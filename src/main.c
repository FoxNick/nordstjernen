...
#define NS_IMAGE_START_GAP_US (220 * G_TIME_SPAN_MILLISECOND)
#define NS_INITIAL_IMAGE_BURST 4
...
static void ns_window_clear_tile_cache(ns_window *w);
static void ns_window_schedule_tile_prewarm(ns_window *w,
                                            int width,
                                            double clip_y,
                                            double clip_h);
static gboolean ns_window_tile_prewarm_cb(gpointer data);
...
static void ns_window_kick_initial_visible_image_loads(ns_window *w);
...
static void
ns_window_clear_tile_cache(ns_window *w)
{
    if (!w) return;
    g_clear_handle_id(&w->tile_prewarm_source, g_source_remove);
...
ns_window_clear_cache(ns_window *w)
{
    ...
    w->last_wheel_us = 0;
    w->last_visible_image_kick_us = 0;
    w->last_image_request_us = 0;
    w->image_backoff_until_us = 0;
    w->last_visible_image_top = 0;
    w->initial_image_burst_done = FALSE;
}
...
ns_window_raf_tick(GtkWidget *widget, GdkFrameClock *clock, gpointer ud)
{
    ...
    if (redraw && w->drawing_area) {
        if (cache_dirty) ns_window_clear_tile_cache(w);
        gtk_widget_queue_draw(w->drawing_area);
    }
    return G_SOURCE_CONTINUE;
}
...
ns_window_ensure_layout(ns_window *w, double viewport_width)
{
    ...
    ns_window_apply_page_title(w);
    ns_window_kick_initial_visible_image_loads(w);
    ns_window_kick_image_loads(w);
...
static void
ns_window_schedule_tile_prewarm(ns_window *w,
                                int width,
                                double clip_y,
                                double clip_h)
{
    if (!w || !w->tile_cache || !w->layout_tree) return;
    if (width <= 0 || clip_h <= 0) return;
    w->tile_prewarm_width = width;
    w->tile_prewarm_y = clip_y;
    w->tile_prewarm_h = clip_h;
    if (!w->tile_prewarm_source)
        w->tile_prewarm_source =
            g_idle_add_full(G_PRIORITY_LOW, ns_window_tile_prewarm_cb,
                            w, NULL);
}

static gboolean
ns_window_tile_prewarm_cb(gpointer data)
{
    ns_window *w = data;
    if (!w) return G_SOURCE_REMOVE;
    w->tile_prewarm_source = 0;
    if (w->mode != NS_VIEW_RENDER || !w->drawing_area ||
        !w->layout_tree || !w->tile_cache)
        return G_SOURCE_REMOVE;
    if (w->focused_input || w->selection.active ||
        ns_selection_has_range(&w->selection))
        return G_SOURCE_REMOVE;
    if (w->tile_prewarm_width <= 0 || w->tile_prewarm_h <= 0)
        return G_SOURCE_REMOVE;
    ...
    guint warmed = ns_tile_cache_prewarm(w->tile_cache,
                                         w->layout_tree,
                                         w->search_query,
                                         w->search_active_box,
                                         state_flags,
                                         w->tile_prewarm_width,
                                         w->tile_prewarm_y,
                                         w->tile_prewarm_h,
                                         1);
    ns_paint_set_cull_margin(400.0);
    if (warmed > 0 && ns_profile_enabled()) {
        ns_tile_cache_stats stats = {0};
        ns_tile_cache_stats_get(w->tile_cache, &stats);
        g_printerr("[profile] tile-prewarm warmed=%u tiles=%u cache=%.1fMB "
                   "y=%.0f h=%.0f dirty=%d pending=%u\n",
                   warmed, stats.tiles,
                   stats.bytes / (1024.0 * 1024.0),
                   w->tile_prewarm_y, w->tile_prewarm_h,
                   w->layout_dirty, w->js_relayout_idle_id);
    }
    return G_SOURCE_REMOVE;
}
               ...
               int width, int height, gpointer user_data)
{
    ...
    if (cacheable) {
        ...
        ns_window_schedule_tile_prewarm(w, width, iy0, iy1 - iy0);
    } else {
        cairo_save(cr);
...
static gboolean
ns_window_invalidate_box_tiles(ns_window *w, const ns_box *box)
{
    if (!w || !box || !w->tile_cache) return FALSE;
    double top = box->paint_top;
    double bottom = box->paint_bottom;
    if (bottom <= top) {
        top = box->y;
...
ns_window_image_slot_available(guint inflight, guint limit,
                               guint started, guint burst_limit)
{
    return inflight < limit && started < burst_limit;
}
...
ns_window_request_image(ns_window *w, const char *src, const char *kind,
                        ...
                        guint *started, guint burst_limit,
                        gboolean bypass_gap, gboolean *deferred)
{
    if (!src || g_str_has_prefix(src, "nd-inline-svg:")) return NULL;
    char *abs = ns_resolve_url(w, src);
    if (!abs) return NULL;
    gint64 now_us = g_get_monotonic_time();
    gboolean local_data = g_str_has_prefix(abs, "data:");
    ns_image *cached = ns_image_cache_peek(w->images, abs);
    if (cached && !ns_image_should_retry(cached, now_us)) {
        g_free(abs);
        return cached;
    }
    guint delay = ns_window_image_next_delay_ms(w, now_us);
    gboolean blocked_by_gap = delay > 0 &&
        (!bypass_gap || w->image_backoff_until_us > now_us);
    if (!local_data &&
        (!ns_window_image_slot_available(*inflight, limit,
                                         *started, burst_limit) ||
         blocked_by_gap)) {
                                       ...
                                       gboolean bypass_gap)
{
    ...
    guint started = 0;
    guint ready_attached = 0;
    guint candidates = 0;
    if (burst_limit == 0) burst_limit = 1;
    gboolean deferred = FALSE;
    gboolean visual_invalidated = FALSE;
    for (guint i = 0; i < imgs->len; i++) {
        ns_box *box = g_ptr_array_index(imgs, i);
        if (!box->media) continue;
        ...
        if (box->media->image_src && !box->media->image) {
            candidates++;
            ns_image *img = ns_window_request_image(w,
                box->media->image_src, "image",
                &inflight, limit, &started, burst_limit,
                bypass_gap, &deferred);
            if (img) {
                box->media->image = img;
                if (img->loaded || img->failed) {
                    ready_attached++;
                    visual_invalidated |= ns_window_invalidate_box_tiles(w, box);
                }
            }
            if (deferred) break;
        }
    ...
    g_ptr_array_free(imgs, TRUE);
    if (visual_invalidated && w->drawing_area)
        gtk_widget_queue_draw(w->drawing_area);
    if (ns_profile_enabled() && (started > 0 || deferred || ready_attached > 0)) {
        g_printerr("[profile] image-kick margin=%.0f burst=%u bypass=%d "
                   "candidates=%u inflight=%u started=%u ready=%u deferred=%d\n",
                   margin, burst_limit, bypass_gap, candidates, inflight,
                   started, ready_attached, deferred);
    }
...
ns_window_kick_image_loads(ns_window *w)
{
    ns_window_kick_image_loads_with_margin(w,
        ns_window_image_prefetch_margin(w), 1, FALSE);
}
...
{
    ns_window_kick_image_loads_with_margin(w,
        ns_window_image_visible_margin(w), 1, TRUE);
}

static void
ns_window_kick_initial_visible_image_loads(ns_window *w)
{
    if (!w || w->initial_image_burst_done) return;
    if (w->css_inflight > 0) return;
    w->initial_image_burst_done = TRUE;
    w->last_visible_image_kick_us = g_get_monotonic_time();
    w->last_visible_image_top = w->render_vadj
        ? gtk_adjustment_get_value(w->render_vadj) : 0;
    double margin = ns_window_visible_page_height(w) * 0.25;
    if (margin < 300) margin = 300;
    if (margin > 900) margin = 900;
    ns_window_kick_image_loads_with_margin(w, margin,
        NS_INITIAL_IMAGE_BURST, TRUE);
}
