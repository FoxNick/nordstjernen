...
static void nd_window_clear_html_drag(nd_window *w);
static int nd_dom_buttons_bit(int dom_button);
...
static void
nd_window_clear_html_drag(nd_window *w)
{
    if (!w) return;
    if (w->html_drag_session)
        nd_js_drag_session_free(w->html_drag_session);
...
#define ND_HISTORY_MAX 512

static void
nd_window_history_append(nd_window *w, const char *url)
{
    if (!w->history) return;
    ...
    while (w->history->len > ND_HISTORY_MAX) {
...
nd_window_js_soft_nav(const char *url, gboolean replace, gpointer user_data)
{
    ...
    if (replace) {
        ...
        nd_window_history_append(w, url);
    }
...
nd_window_maybe_submit_form(nd_window *w, const nd_node *clicked)
{
    ...
    if (is_post) {
        ...
        nd_window_history_append(w, abs_action);
...
nd_window_apply_page_title(nd_window *w)
{
    ...
    if (trimmed->len > 0 && w->history && w->cursor >= 0 &&
        ...
        if (cur) nd_history_record(cur, trimmed->str);
    }
...
static gboolean
nd_rel_has_token(const char *rel, const char *token)
{
...
static gboolean
nd_rel_is_stylesheet(const char *rel)
{
    return nd_rel_has_token(rel, "stylesheet") &&
           !nd_rel_has_token(rel, "alternate");
}
...
static void
nd_window_store_external_stylesheet_group(nd_window *w, const char *url,
                                          guint first_index)
{
...
static void
nd_window_append_owned_stylesheet_range(GPtrArray *page_sheets,
...
static const char *
nd_window_node_base_url(nd_window *w, const nd_node *n)
{
    for (const nd_node *a = n; a; a = a->parent) {
        if (!nd_node_is_element_named(a, "iframe")) continue;
        const char *furl = nd_element_get_attr(a, "data-nd-frame-url");
        if (furl && *furl) return furl;
    }
    return nd_window_current_url(w);
}

static char *
nd_window_resolve_for_node(nd_window *w, const nd_node *n, const char *href)
{
    const char *base = nd_window_node_base_url(w, n);
    if (base != nd_window_current_url(w))
        return nd_url_resolve(base, href);
    return nd_resolve_url(w, href);
}
...
typedef struct {
    ...
    GString    *run;
    const char *run_base;
} nd_sheet_collect_ctx;

static void
nd_window_sheet_run_flush(nd_sheet_collect_ctx *cc)
{
    if (!cc->run || cc->run->len == 0) return;
    nd_css_stylesheet *sh =
        nd_css_merged_styles_cached(cc->run->str, (gssize)cc->run->len);
    if (sh) {
        GHashTable *seen =
            g_hash_table_new_full(g_str_hash, g_str_equal, g_free, NULL);
        guint first = cc->page_sheets->len;
        nd_window_append_stylesheet_expanded(cc->w, cc->page_sheets, sh,
                                             cc->run_base, seen, 0);
        nd_window_append_owned_stylesheet_range(cc->page_sheets,
...
static void
nd_window_collect_page_stylesheets_walk(nd_node *n, nd_sheet_collect_ctx *cc)
{
    if (!n || nd_node_is_element_named(n, "noscript")) return;
    nd_window *w = cc->w;
    if (nd_node_is_element_named(n, "iframe"))
        nd_window_sheet_run_flush(cc);
    if (nd_node_is_element_named(n, "style")) {
        char *css = nd_css_style_element_text(n);
        if (css) {
            const char *base = nd_window_node_base_url(w, n);
            ...
            if (strstr(css, "@import")) {
                nd_window_sheet_run_flush(cc);
                nd_css_stylesheet *sh =
                    nd_css_stylesheet_from_style_element_cached(n);
                if (sh) {
                    GHashTable *seen =
                        g_hash_table_new_full(g_str_hash, g_str_equal,
                                              g_free, NULL);
                    guint first = cc->page_sheets->len;
                    nd_window_append_stylesheet_expanded(w, cc->page_sheets,
                                                         sh, base, seen, 0);
                    nd_window_append_owned_stylesheet_range(cc->page_sheets,
                                                            cc->owned_sheets,
                                                            first);
    ...
    } else if (nd_node_is_element_named(n, "link")) {
        ...
        if (href && *href && nd_rel_is_stylesheet(rel) &&
            (!media || !*media || nd_css_media_query_matches(media))) {
            nd_window_sheet_run_flush(cc);
            char *abs = nd_window_resolve_for_node(w, n, href);
    ...
    for (nd_node *c = n->first_child; c; c = c->next_sibling)
        nd_window_collect_page_stylesheets_walk(c, cc);
}

static void
nd_window_collect_page_stylesheets(nd_window *w, nd_node *doc,
                                   GPtrArray *page_sheets,
                                   GPtrArray *owned_sheets)
{
    if (!w || !doc) return;
    nd_sheet_collect_ctx cc = {
        .w = w, .page_sheets = page_sheets, .owned_sheets = owned_sheets,
        .run = g_string_new(NULL), .run_base = NULL,
    };
    nd_window_collect_page_stylesheets_walk(doc, &cc);
    nd_window_sheet_run_flush(&cc);
...
nd_window_ensure_layout(nd_window *w, double viewport_width)
{
    ...
    nd_css_style_element_cache_begin();
    ...
    nd_window_collect_page_stylesheets(w, w->parsed_doc, page_sheets,
                                       owned_sheets);
    ...
    for (guint i = 0; i < owned_sheets->len; i++)
        nd_css_stylesheet_free(g_ptr_array_index(owned_sheets, i));
    ...
    w->last_viewport_w = viewport_width;
    {
        ...
        nd_debug_log_emit(ND_DLOG_RENDER, "layout",
            ...
            "total=%.1fms",
            viewport_width, style_count, (guint)sheet_count,
...
nd_window_event_target_at(nd_window *w, double x, double y)
{
...
static void
nd_window_client_xy(nd_window *w, double x, double y, double *cx, double *cy)
{
    double sx = 0, sy = 0;
...
static gboolean
nd_window_dispatch_mouse(nd_window *w, const nd_node *node, const char *type,
                         double x, double y, int button, int buttons,
                         GdkModifierType state, const nd_node *related)
{
    if (!w->js || !node) return FALSE;
    double cx, cy;
    nd_window_client_xy(w, x, y, &cx, &cy);
    gboolean prevented = FALSE;
    nd_js_dispatch_mouse_event(w->js, node, type, cx, cy, x, y, button, buttons,
...
static void
nd_window_emit_pointer_and_mouse(nd_window *w, const nd_node *node,
                                 const char *pointer_type, const char *mouse_type,
                                 double x, double y, int button, int buttons,
                                 GdkModifierType state, const nd_node *related)
{
    if (!w->js || !node) return;
    nd_window_dispatch_mouse(w, node, pointer_type, x, y, button, buttons,
                             state, related);
    if (!w->js) return;
    nd_window_dispatch_mouse(w, node, mouse_type, x, y, button, buttons,
                             state, related);
}

static gboolean
nd_element_attr_is_true(const nd_node *n, const char *name)
{
    const char *v = nd_element_get_attr(n, name);
    return v && g_ascii_strcasecmp(v, "true") == 0;
}

static gboolean
nd_element_attr_is_false(const nd_node *n, const char *name)
{
    const char *v = nd_element_get_attr(n, name);
...
static const nd_node *
nd_window_html_drag_source_at(nd_window *w, double x, double y)
{
    const nd_node *hit = nd_window_event_target_at(w, x, y);
    for (const nd_node *p = hit; p; p = p->parent) {
        if (p->kind != ND_NODE_ELEMENT || !p->name) continue;
        if (nd_element_effectively_inert(p) ||
            nd_element_effectively_disabled(p))
            return NULL;
        if (nd_element_attr_is_true(p, "draggable"))
            return p;
        if (nd_element_attr_is_false(p, "draggable"))
            continue;
        if (strcmp(p->name, "a") == 0) {
            const char *href = nd_element_get_attr(p, "href");
            if (href && *href) return p;
        }
        if (strcmp(p->name, "img") == 0) {
            const char *src = nd_element_get_attr(p, "src");
            if (src && *src) return p;
        }
    }
    return NULL;
...
static const nd_node *
nd_window_html_drag_target_at(nd_window *w, double x, double y)
{
    const nd_node *hit = nd_window_event_target_at(w, x, y);
    if (hit) return hit;
...
static void
nd_window_seed_html_drag_data(nd_window *w, const nd_node *source)
{
    if (!w || !source || !w->html_drag_session) return;
    const char *raw = NULL;
    if (nd_node_is_element_named(source, "a"))
        raw = nd_element_get_attr(source, "href");
    else if (nd_node_is_element_named(source, "img"))
        raw = nd_element_get_attr(source, "src");
    if (!raw || !*raw) return;
    char *abs = nd_resolve_url(w, raw);
    if (!abs) return;
    nd_js_drag_session_set_data(w->html_drag_session, "text/plain", abs);
    nd_js_drag_session_set_data(w->html_drag_session, "text/uri-list", abs);
    g_free(abs);
}

static gboolean
nd_window_dispatch_drag(nd_window *w, const nd_node *target, const char *type,
                        ...
                        GdkModifierType state, const nd_node *related)
{
    ...
    nd_window_client_xy(w, x, y, &cx, &cy);
    gboolean prevented = FALSE;
    nd_js_dispatch_drag_event(w->js, w->html_drag_session, target, type,
                              cx, cy, x, y, 0, buttons,
                              (state & GDK_SHIFT_MASK)   != 0,
                              (state & GDK_CONTROL_MASK) != 0,
                              (state & GDK_ALT_MASK)     != 0,
...
static gboolean
nd_window_begin_html_drag(nd_window *w, GtkGestureDrag *gesture,
                          double x, double y)
{
    nd_window_clear_html_drag(w);
    if (!w || !w->js || !w->layout_tree) return FALSE;
    const nd_node *source = nd_window_html_drag_source_at(w, x, y);
    if (!source) return FALSE;
    w->html_drag_session = nd_js_drag_session_new(w->js);
    if (!w->html_drag_session) return FALSE;
    w->html_drag_source = source;
    nd_window_seed_html_drag_data(w, source);
    GdkModifierType state = gtk_event_controller_get_current_event_state(
        GTK_EVENT_CONTROLLER(gesture));
    gboolean prevented = nd_window_dispatch_drag(w, source, "dragstart",
                                                 x, y, nd_dom_buttons_bit(0),
                                                 state, NULL);
    if (prevented) {
        nd_window_clear_html_drag(w);
        return FALSE;
...
static gboolean
nd_window_update_html_drag(nd_window *w, GtkGestureDrag *gesture,
                           double x, double y)
{
    if (!w || !w->html_drag_active || !w->html_drag_session) return FALSE;
    GdkModifierType state = gtk_event_controller_get_current_event_state(
        GTK_EVENT_CONTROLLER(gesture));
    const nd_node *target = nd_window_html_drag_target_at(w, x, y);
    if (target != w->html_drag_over) {
        const nd_node *old = w->html_drag_over;
        w->html_drag_can_drop = FALSE;
        if (old)
            nd_window_dispatch_drag(w, old, "dragleave", x, y,
                                    nd_dom_buttons_bit(0), state, target);
        w->html_drag_over = target;
        if (target) {
            gboolean enter_prevented =
                nd_window_dispatch_drag(w, target, "dragenter", x, y,
                                        nd_dom_buttons_bit(0), state, old);
            if (enter_prevented) w->html_drag_can_drop = TRUE;
        }
    }
    if (target) {
        gboolean over_prevented =
            nd_window_dispatch_drag(w, target, "dragover", x, y,
                                    nd_dom_buttons_bit(0), state, NULL);
...
static gboolean
nd_window_end_html_drag(nd_window *w, GtkGestureDrag *gesture,
                        double x, double y)
{
    if (!w || !w->html_drag_active || !w->html_drag_session) return FALSE;
    GdkModifierType state = gtk_event_controller_get_current_event_state(
    ...
    if (target && target != w->html_drag_over) {
        nd_window_update_html_drag(w, gesture, x, y);
        target = nd_window_html_drag_target_at(w, x, y);
    }
    if (target && w->html_drag_can_drop)
        nd_window_dispatch_drag(w, target, "drop", x, y, 0, state, NULL);
    if (w->html_drag_source)
        nd_window_dispatch_drag(w, w->html_drag_source, "dragend",
                                x, y, 0, state, target);
    nd_window_clear_html_drag(w);
    return TRUE;
}
...
static int
nd_dom_buttons_bit(int dom_button)
{
                      ...
                      double x, double y, gpointer user_data)
{
    ...
    GdkModifierType pstate = gtk_event_controller_get_current_event_state(
        GTK_EVENT_CONTROLLER(gesture));
    if (w->js && w->layout_tree) {
        const nd_node *down = nd_window_event_target_at(w, x, y);
        if (down) {
            nd_window_emit_pointer_and_mouse(w, down, "pointerdown", "mousedown",
                                             x, y, 0, nd_dom_buttons_bit(0),
                                             pstate, NULL);
            if (n_press == 2 && w->js && w->layout_tree) {
                const nd_node *dbl = nd_window_event_target_at(w, x, y);
                if (dbl)
                    nd_window_dispatch_mouse(w, dbl, "dblclick", x, y, 0, 0,
                                             pstate, NULL);
            }
        }
    }
    ...
    if (!link) {
        ...
        if (form_target) {
            ...
            gboolean prevented = nd_window_dispatch_mouse(w, form_target, "click",
                                                          x, y, 0, 0, pstate, NULL);
            if (!prevented) {
        ...
        if (hit && hit->dom) {
            ...
            gboolean prevented = nd_window_dispatch_mouse(w, hit->dom, "click",
                                                          x, y, 0, 0, pstate, NULL);
            if (!prevented) {
                char *media_abs = NULL;
    ...
    if (w->js && link->dom) {
        if (nd_window_dispatch_mouse(w, link->dom, "click", x, y, 0, 0, mods, NULL))
            return;
    }
...
void
nd_on_drawing_released(GtkGestureClick *gesture, int n_press,
                       double x, double y, gpointer user_data)
{
    (void)n_press;
    nd_window *w = user_data;
    if (!w->js || !w->layout_tree) return;
    const nd_node *up = nd_window_event_target_at(w, x, y);
    if (!up) return;
    GdkModifierType state = gtk_event_controller_get_current_event_state(
        GTK_EVENT_CONTROLLER(gesture));
    nd_window_emit_pointer_and_mouse(w, up, "pointerup", "mouseup",
                                     x, y, 0, 0, state, NULL);
}
...
nd_on_paste_text(GObject *source, GAsyncResult *result, gpointer user_data)
{
    ...
    if (text && w && nd_window_focused_input_live(w)) {
...
nd_window_input_paste(nd_window *w)
{
    ...
    gdk_clipboard_read_text_async(cb, NULL, nd_on_paste_text,
...
static int
nd_keyval_to_js_keycode(guint keyval)
{
...
static const char *
nd_keyval_to_js_code(guint keyval, char *buf, gsize buflen)
{
    if (keyval >= GDK_KEY_a && keyval <= GDK_KEY_z) {
                             ...
                             GdkModifierType state, GdkEvent *event)
{
    ...
    char code_buf[16];
    const char *code = nd_keyval_to_js_code(keyval, code_buf, sizeof code_buf);
    if (!code) code = name ? name : "";
    gboolean prevented = FALSE;
    nd_js_dispatch_key_event(w->js, target, type, key, code,
                             nd_keyval_to_js_keycode(keyval),
                             (state & GDK_SHIFT_MASK)   != 0,
                             (state & GDK_CONTROL_MASK) != 0,
                             (state & GDK_ALT_MASK)     != 0,
                             (state & GDK_META_MASK)    != 0,
...
static void nd_window_after_zoom(nd_window *w);
                     ...
                     gpointer user_data)
{
    ...
    if (w->js) {
        const nd_node *wt = nd_window_event_target_at(w, w->cursor_x, w->cursor_y);
        if (wt) {
            double cx, cy;
            nd_window_client_xy(w, w->cursor_x, w->cursor_y, &cx, &cy);
            gboolean prevented = FALSE;
            nd_js_dispatch_wheel_event(w->js, wt, cx, cy,
...
void
nd_on_drawing_side_pressed(GtkGestureClick *gesture, int n_press,
                           double x, double y, gpointer user_data)
{
    ...
    if (button == 8) {
        ...
        w->cursor--;
        nd_window_load_url(w, g_ptr_array_index(w->history, w->cursor),
                           ND_LOAD_HISTORY);
    } else if (button == 9) {
...
void
nd_on_drawing_zoom_begin(GtkGesture *gesture, GdkEventSequence *seq,
                         gpointer user_data)
{
...
void
nd_on_drawing_zoom_end(GtkGesture *gesture, GdkEventSequence *seq,
                       gpointer user_data)
{
    (void)seq;
    nd_window *w = user_data;
                         ...
                         gpointer user_data)
{
    ...
    w->drag_start_y = y;
    if (!w->layout_tree) return;
    if (nd_window_begin_html_drag(w, gesture, x, y)) {
        nd_selection_clear(&w->selection);
        nd_window_sync_selection_to_js(w);
        if (w->drawing_area) gtk_widget_queue_draw(w->drawing_area);
        return;
...
{
    nd_window *w = user_data;
    if (nd_window_update_html_drag(w, gesture,
                                   w->drag_start_x + dx,
                                   w->drag_start_y + dy))
        return;
...
{
    nd_window *w = user_data;
    if (nd_window_end_html_drag(w, gesture,
                                w->drag_start_x + dx,
                                w->drag_start_y + dy))
        return;
    if (!w->layout_tree || !w->selection.active) return;
...
static void
nd_css_fetch_free(nd_css_fetch *fetch)
{
    if (!fetch) return;
    g_free(fetch->url);
...
static void
nd_window_css_fetch_done(nd_window *w)
{
    if (!w) return;
...
static void
on_external_css_parsed(nd_tab_css_result *parsed, gpointer user_data)
{
    nd_css_fetch *fetch = user_data;
    nd_window *w = nd_window_for_id(fetch->w_id);
    if (!w || fetch->gen != w->fetch_gen) {
        nd_tab_css_result_free(parsed);
        nd_css_fetch_free(fetch);
        return;
    }
    if (parsed && parsed->sheet && w->external_stylesheets) {
        ...
        nd_window_append_stylesheet_expanded(w, w->external_stylesheets,
                                             ...
                                             seen, 0);
        parsed->sheet = NULL;
        nd_window_store_external_stylesheet_group(w, fetch->url, first);
        g_hash_table_destroy(seen);
        nd_window_mark_layout_dirty(w);
    }
    nd_tab_css_result_free(parsed);
    nd_window_css_fetch_done(w);
    nd_css_fetch_free(fetch);
}
...
on_external_css_loaded(GObject *src, GAsyncResult *result, gpointer user_data)
{
    ...
    if (resp->error) {
        ...
        if (w->worker &&
            nd_tab_worker_parse_css_response(w->worker, resp, fetch->scope_id,
                                             on_external_css_parsed, fetch,
                                             (GDestroyNotify)nd_css_fetch_free))
            return;
        nd_tab_css_result *parsed = g_new0(nd_tab_css_result, 1);
        parsed->resp = resp;
        ...
        if (lt + 5 < end && g_ascii_strncasecmp(lt, "<link", 5) == 0 &&
            ...
            if (href && *href && nd_rel_is_stylesheet(rel)) {
                ...
                if (abs && !nd_window_subresource_blocked(
                    ...
                    w->css_inflight++;
                    nd_net_fetch_async(abs, nd_window_current_url(w),
                                       w->css_cancellable,
...
nd_window_record_final_url(nd_window *w, const nd_response *resp)
{
    ...
    nd_history_record(resp->final_url, NULL);
}
...
static nd_tab_load_result *
nd_prepare_load_sync(nd_response *resp, gboolean parse_html)
{
    nd_tab_load_result *prepared = g_new0(nd_tab_load_result, 1);
    prepared->resp = resp;
    if (resp && resp->body && resp->body->len > 0) {
        prepared->body = nd_html_decode_body((const char *)resp->body->data,
                                             resp->body->len);
        if (!prepared->body) prepared->body = g_strdup("");
        prepared->body_len = strlen(prepared->body);
        if (parse_html) {
            prepared->doc = nd_html_parse(prepared->body,
...
static void
nd_on_load_prepared(nd_tab_load_result *prepared, gpointer user_data)
{
    ...
    if (!w || gen != w->fetch_gen || !prepared || !prepared->resp) {
        nd_tab_load_result_free(prepared);
        return;
    }

    nd_response *resp = prepared->resp;
    if (prepared->body) {
    ...
    if (is_html && w->last_body)
        nd_window_preload_stylesheets(w, w->last_body, w->last_body_len);
...
nd_on_fetch_done(GObject *src, GAsyncResult *result, gpointer user_data)
{
    ...
    if (w->worker &&
        nd_tab_worker_load_response(w->worker, resp, parse_html,
                                    nd_on_load_prepared, ctx, g_free))
        return;
    nd_tab_load_result *prepared = nd_prepare_load_sync(resp, parse_html);
    nd_on_load_prepared(prepared, ctx);
}
...
static gboolean
nd_window_load_url_deferred(gpointer data)
{
    nd_deferred_nav *d = data;
    nd_window *w = nd_window_for_id(d->wid);
    if (w) nd_window_load_url(w, d->url, d->src);
    g_free(d->url);
    g_free(d);
    return G_SOURCE_REMOVE;
}
...
{
    if (w->js && nd_js_in_pump(w->js)) {
        nd_deferred_nav *d = g_new0(nd_deferred_nav, 1);
        d->wid = w->id;
        d->url = g_strdup(raw_url);
        d->src = src;
        g_idle_add(nd_window_load_url_deferred, d);
        return;
    }
...
static void
nd_draw_hourglass(cairo_t *cr, double s)
{
...
nd_window_set_busy(nd_window *w, gboolean busy)
{
    ...
    GdkCursor *busy_cursor = busy ? nd_busy_cursor() : NULL;
...
#define ND_TYPE_SUGGEST (nd_suggest_get_type())
G_DECLARE_FINAL_TYPE(NdSuggest, nd_suggest, ND, SUGGEST, GObject)
struct _NdSuggest { GObject parent_instance; char *url; char *title; };
G_DEFINE_FINAL_TYPE(NdSuggest, nd_suggest, G_TYPE_OBJECT)

static void
nd_suggest_finalize(GObject *obj)
{
    NdSuggest *s = ND_SUGGEST(obj);
    g_free(s->url);
    g_free(s->title);
    G_OBJECT_CLASS(nd_suggest_parent_class)->finalize(obj);
}

static void
nd_suggest_class_init(NdSuggestClass *klass)
{
    G_OBJECT_CLASS(klass)->finalize = nd_suggest_finalize;
}

static void
nd_suggest_init(NdSuggest *s) { (void)s; }

static NdSuggest *
nd_suggest_new(const char *url, const char *title)
{
    NdSuggest *s = g_object_new(ND_TYPE_SUGGEST, NULL);
    s->url   = g_strdup(url);
...
static void
nd_window_hide_suggestions(nd_window *w)
{
...
on_url_focus_leave(GtkEventControllerFocus *ctrl, gpointer user_data)
{
    ...
    nd_window *w = user_data;
    w->url_focused = FALSE;
    nd_window_hide_suggestions(w);
}

static void
nd_suggest_navigate(nd_window *w, const char *url)
{
    if (!url || !*url) return;
    char *u = g_strdup(url);
    ...
    if (w->drawing_area) gtk_widget_grab_focus(w->drawing_area);
    nd_window_load_url(w, u, ND_LOAD_USER);
    g_free(u);
}
...
on_suggest_row_activated(GtkListView *list, guint position, gpointer user_data)
{
    ...
    NdSuggest *s = g_list_model_get_item(G_LIST_MODEL(w->suggest_model), position);
    if (s) {
        char *u = g_strdup(s->url);
        g_object_unref(s);
        nd_suggest_navigate(w, u);
        g_free(u);
    }
}

static void
suggest_move_selection(nd_window *w, int delta)
{
...
static guint
suggest_selected_position(nd_window *w)
{
...
on_url_entry_changed(GtkEditable *editable, gpointer user_data)
{
    ...
    GPtrArray *matches = (text && *text) ? nd_history_suggest(text, 8) : NULL;
    ...
    for (guint i = 0; i < matches->len; i++) {
        nd_history_suggestion *m = g_ptr_array_index(matches, i);
        NdSuggest *s = nd_suggest_new(m->url, m->title);
...
void
nd_window_setup_url_suggestions(nd_window *w)
{
    nd_install_suggest_css();
    w->suggest_model = g_list_store_new(ND_TYPE_SUGGEST);
...
on_drawing_motion(GtkEventControllerMotion *ctrl, double x, double y, gpointer user_data)
{
    ...
    if (w->js && w->layout_tree) {
        ...
        const nd_node *now = nd_window_event_target_at(w, x, y);
        const nd_node *prev = w->hover_node;
        if (now != prev) {
            w->hover_node = now;
            if (prev)
                nd_window_emit_pointer_and_mouse(w, prev, "pointerout", "mouseout",
                                                 x, y, 0, 0, st, now);
            if (w->js && w->layout_tree && prev)
                nd_window_emit_pointer_and_mouse(w, prev, "pointerleave",
                                                 "mouseleave", x, y, 0, 0, st, now);
            if (w->js && w->layout_tree && now)
                nd_window_emit_pointer_and_mouse(w, now, "pointerover", "mouseover",
                                                 x, y, 0, 0, st, prev);
            if (w->js && w->layout_tree && now)
                nd_window_emit_pointer_and_mouse(w, now, "pointerenter",
                                                 "mouseenter", x, y, 0, 0, st, prev);
        }
        if (w->js && w->layout_tree && now)
            nd_window_emit_pointer_and_mouse(w, now, "pointermove", "mousemove",
                                             x, y, 0, 0, st, NULL);
    }
}
...
on_window_destroy(GtkWidget *widget, gpointer user_data)
{
    ...
    if (w->worker) nd_tab_worker_free(w->worker);
...
static nd_window *
nd_browser_construct_tab(GtkWidget *toplevel)
{
    ...
    w->worker = nd_tab_worker_new(worker_name);
    w->images  = nd_image_cache_new();
    w->videos  = nd_video_cache_new();
    nd_image_cache_set_worker(w->images, w->worker);
    nd_video_cache_set_worker(w->videos, w->worker);
    w->anim    = nd_anim_new();
    w->zoom    = 1.0;
...
{
    (void)app;
    nd_window *w = nd_browser_construct_tab(toplevel);
    if (!w) return NULL;
...
static nd_window *
nd_browser_add_tab_unloaded(GtkWidget *toplevel, const char *url)
{
    nd_window *w = nd_browser_construct_tab(toplevel);
    if (!w) return NULL;
    if (url && *url) {
...
static void nd_push_closed_url(const char *url);

static gboolean
nd_browser_close_tab_deferred(gpointer data)
{
    nd_window *w = nd_window_for_id(GPOINTER_TO_UINT(data));
    if (w) nd_browser_close_tab(w);
    return G_SOURCE_REMOVE;
}
...
nd_browser_close_tab(nd_window *w)
{
    ...
    if (w->js && nd_js_in_pump(w->js)) {
        g_idle_add(nd_browser_close_tab_deferred, GUINT_TO_POINTER(w->id));
        return;
    }
    GtkWidget *toplevel = w->window;
    nd_push_closed_url(nd_window_current_url(w));
    GPtrArray *tabs = g_object_get_data(G_OBJECT(toplevel), "nd-tabs");
    GtkStack *stack = g_object_get_data(G_OBJECT(toplevel), "nd-stack");
    GtkBox   *strip = g_object_get_data(G_OBJECT(toplevel), "nd-tab-strip");
    if (!tabs) return;
...
static void
nd_app_menu_popup(GtkWidget *toplevel)
{
...
static gboolean
nd_on_menu_key_pressed(GtkEventControllerKey *c, guint keyval, guint keycode,
                       GdkModifierType state, gpointer user_data)
{
    (void)c; (void)keycode;
    GtkWidget *toplevel = user_data;
...
static gboolean
nd_on_menu_key_released(GtkEventControllerKey *c, guint keyval, guint keycode,
                        GdkModifierType state, gpointer user_data)
{
    ...
    nd_app_menu_popup(toplevel);
    return TRUE;
}

static gboolean
nd_toplevel_close_retry(gpointer data)
{
    GtkWindow *win = data;
...
on_toplevel_close_request(GtkWindow *toplevel, gpointer user_data)
{
    ...
    for (guint i = 0; i < tabs->len; i++) {
        nd_window *w = g_ptr_array_index(tabs, i);
        if (w && w->js && nd_js_in_pump(w->js)) {
            g_idle_add(nd_toplevel_close_retry, g_object_ref(toplevel));
            return TRUE;
        }
    }
    return FALSE;
...
static GtkWidget *
nd_browser_open_shell(GtkApplication *app)
{
    ...
    g_signal_connect(menukey, "key-pressed", G_CALLBACK(nd_on_menu_key_pressed), toplevel);
    g_signal_connect(menukey, "key-released", G_CALLBACK(nd_on_menu_key_released), toplevel);
...
{
    GtkWidget *toplevel = nd_browser_open_shell(app);
...
#define ND_SESSION_WRITE_SECS 3

#ifndef G_OS_WIN32
static gboolean
nd_on_quit_signal(gpointer user_data)
{
...
static gboolean
nd_session_url_restorable(const char *url)
{
...
static gboolean
nd_session_write_cb(gpointer user_data)
{
    ...
    for (GList *l = gtk_application_get_windows(app); l; l = l->next) {
        ...
        for (guint i = 0; i < tabs->len; i++) {
            nd_window *t = g_ptr_array_index(tabs, i);
            const char *url = t->lazy_url ? t->lazy_url : nd_window_current_url(t);
            if (nd_session_url_restorable(url))
...
static void
nd_session_recovery_notice(GtkWindow *parent, guint n)
{
    GtkAlertDialog *dlg =
...
static gboolean
nd_session_try_restore(GtkApplication *app)
{
    char *contents = NULL;
    ...
    for (int i = 0; lines[i]; i++) {
        ...
        if (nd_session_url_restorable(u))
    ...
    GtkWidget *toplevel = nd_browser_open_shell(app);
    nd_window *first = NULL;
    for (guint i = 0; i < urls->len; i++) {
        nd_window *w = nd_browser_add_tab_unloaded(toplevel,
                                                   g_ptr_array_index(urls, i));
        if (w && !first) first = w;
    }
    gtk_window_maximize(GTK_WINDOW(toplevel));
    gtk_window_present(GTK_WINDOW(toplevel));
    if (first) nd_browser_set_active(toplevel, first);
    nd_session_recovery_notice(GTK_WINDOW(toplevel), urls->len);
...
on_activate(GtkApplication *app, gpointer user_data)
{
    ...
    if (!recovery_consumed && nd_watchdog_child_is_recovery()) {
        recovery_consumed = TRUE;
        restored = nd_session_try_restore(app);
    }
    if (!restored) {
...
static gboolean
nd_window_after_zoom_deferred(gpointer data)
{
    nd_window *w = nd_window_for_id(GPOINTER_TO_UINT(data));
    if (w) nd_window_after_zoom(w);
    return G_SOURCE_REMOVE;
}
...
{
    if (w->js && nd_js_in_pump(w->js)) {
        g_idle_add(nd_window_after_zoom_deferred, GUINT_TO_POINTER(w->id));
        return;
    }
    if (w->layout_tree) { if (w->js) nd_js_set_layout_root(w->js, NULL); nd_box_free(w->layout_tree); w->layout_tree = NULL; nd_selection_clear(&w->selection); w->search_active_box = NULL; }
    if (w->style_table) { if (w->js) nd_js_set_style_table(w->js, NULL); g_hash_table_destroy(w->style_table); w->style_table = NULL; }
...
static void
nd_push_closed_url(const char *url)
{
...
static void
nd_browser_switch_to_index(GtkWidget *toplevel, int index)
{
    GPtrArray *tabs = g_object_get_data(G_OBJECT(toplevel), "nd-tabs");
    if (!tabs || tabs->len == 0) return;
    if (index < 0) index = 0;
    if (index >= (int)tabs->len) index = (int)tabs->len - 1;
    nd_browser_set_active(toplevel, g_ptr_array_index(tabs, index));
}

static void
nd_browser_switch_relative(GtkWidget *toplevel, int delta)
{
    GPtrArray *tabs = g_object_get_data(G_OBJECT(toplevel), "nd-tabs");
    nd_window *cur = g_object_get_data(G_OBJECT(toplevel), "nd-window");
    if (!tabs || tabs->len == 0 || !cur) return;
    guint idx = 0;
    if (!g_ptr_array_find(tabs, cur, &idx)) return;
    int n = (int)tabs->len;
    int next = (((int)idx + delta) % n + n) % n;
    nd_browser_set_active(toplevel, g_ptr_array_index(tabs, next));
}
...
on_win_hard_reload(GSimpleAction *action, GVariant *parameter, gpointer user_data)
{
    ...
    if (w->cursor < 0 || w->cursor >= (int)w->history->len) return;
    const char *cur = g_ptr_array_index(w->history, w->cursor);
    nd_window_load_url(w, cur, ND_LOAD_USER);
}
...
on_win_next_tab(GSimpleAction *action, GVariant *parameter, gpointer user_data)
{
    (void)action; (void)parameter;
    nd_window *w = user_data;
    nd_browser_switch_relative(w->window, +1);
}
...
on_win_select_tab(GSimpleAction *action, GVariant *parameter, gpointer user_data)
{
    ...
    if (!tabs) return;
    if (n >= 9) nd_browser_switch_to_index(w->window, (int)tabs->len - 1);
    else        nd_browser_switch_to_index(w->window, n - 1);
}
...
main(int argc, char **argv)
{
    ...
    nd_config_init();
    if (nd_watchdog_should_supervise(argc, argv,
                                     nd_config_get()->watchdog_enabled)) {
        int rc = nd_watchdog_run_supervisor(g_self_exe, argc, argv);
        nd_config_shutdown();
        return rc;
    }
    ...
    if (nd_watchdog_is_child(argc, argv)) {
        nd_watchdog_child_guard_parent_death();
        nd_watchdog_child_arm_hang_monitor(nd_config_get()->js_eval_budget_ms);
#ifndef G_OS_WIN32
        g_unix_signal_add(SIGTERM, nd_on_quit_signal, app);
        g_unix_signal_add(SIGINT, nd_on_quit_signal, app);
#endif
    }
    g_watchdog_session_path = g_strdup(nd_watchdog_child_session_arg(argc, argv));
    if (g_watchdog_session_path && *g_watchdog_session_path)
        g_timeout_add_seconds(ND_SESSION_WRITE_SECS, nd_session_write_cb, app);
