...
typedef struct ns_webgl_prompt_ctx {
    char      *origin;
    GtkWindow *parent;
} ns_webgl_prompt_ctx;
...
static void
ns_webgl_prompt_ctx_free(ns_webgl_prompt_ctx *ctx)
{
    g_free(ctx->origin);
    g_free(ctx);
}
...
static void
ns_webgl_first_done(GObject *src, GAsyncResult *res, gpointer ud)
{
    ns_webgl_prompt_ctx *ctx = ud;
    GError *err = NULL;
    int idx = gtk_alert_dialog_choose_finish(GTK_ALERT_DIALOG(src), res, &err);
    if (err) { idx = 0; g_error_free(err); }
    gboolean allow = idx == 1;
    ns_webgl_record_decision(ctx->origin, allow);
    if (allow) {
        ...
        if (ctx->parent)
            g_action_group_activate_action(G_ACTION_GROUP(ctx->parent),
                                           "reload", NULL);
    }
    ns_webgl_prompt_ctx_free(ctx);
}

static void
ns_webgl_prompt_async(const char *origin, gboolean already_enabled)
{
    GApplication *app = g_application_get_default();
    GtkWindow *parent = (app && GTK_IS_APPLICATION(app))
        ? gtk_application_get_active_window(GTK_APPLICATION(app)) : NULL;

    ns_webgl_prompt_ctx *ctx = g_new0(ns_webgl_prompt_ctx, 1);
    ctx->origin = g_strdup(origin);
    ctx->parent = parent;

    char *primary = already_enabled
        ? g_strdup_printf("Allow WebGL for %s?", origin)
        : g_strdup("Enable WebGL?");
    char *detail = g_strdup_printf(
        "This page wants to use WebGL (hardware-accelerated 3D graphics) "
    ...
    gtk_alert_dialog_choose(dlg, parent, NULL, ns_webgl_first_done, ctx);
    g_object_unref(dlg);
    g_free(primary);
    g_free(detail);
}
...
ns_webgl_permission(ns_js *js)
{
    ...
    g_hash_table_insert(g_webgl_decisions, g_strdup(origin),
                        GINT_TO_POINTER(3));
    ns_webgl_prompt_async(origin, cfg && cfg->webgl_enabled);
    g_free(origin);
    return FALSE;
}
...
ns_webgl_alloc_storage(ns_webgl *g, int w, int h)
{
    ...
    if (g->samples > 1) {
        glBindFramebuffer(GL_FRAMEBUFFER, g->draw_fbo);
              ...
              JSValueConst attrs)
{
    ...
    if (!gl) { g_clear_error(&err); return NULL; }
    gdk_gl_context_set_allowed_apis(gl, GDK_GL_API_GLES);
    if (!gdk_gl_context_realize(gl, &err)) {
        g_clear_error(&err);
        g_object_unref(gl);
        return NULL;
    }
...
static JSValue wgl_deleteBuffer(JSContext *c, JSValueConst t, int a, JSValueConst *v)
{ return wgl_del_obj(c, t, a, v, glDeleteBuffers); }

static JSValue
wgl_bindBuffer(JSContext *ctx, JSValueConst this_val, int argc, JSValueConst *argv)
{
    WGL_GET(0);
    glBindBuffer((GLenum)argi(ctx, argc, argv, 0), (GLuint)wgl_name(ctx, argv[1]));
    return JS_UNDEFINED;
}

static gboolean
wgl_buffer_range_ok(GLenum target, GLintptr offset, size_t len)
{
...
wgl_bufferSubData(JSContext *ctx, JSValueConst this_val, int argc, JSValueConst *argv)
{
    ...
    if (p) {
        if (wgl_buffer_range_ok(target, offset, len))
...
static gboolean
wgl_elements_in_range(GLsizei count, GLenum type, GLintptr offset)
{
...
wgl_drawElements(JSContext *ctx, JSValueConst this_val, int argc, JSValueConst *argv)
{
    ...
    if (!wgl_elements_in_range(count, type, offset)) return JS_UNDEFINED;
...
wgl_readPixels(JSContext *ctx, JSValueConst this_val, int argc, JSValueConst *argv)
{
    ...
    if (p && len >= need && need > 0) {
        ...
        if (g->samples > 1 && (GLuint)bound == g->draw_fbo) {
            glBindFramebuffer(GL_READ_FRAMEBUFFER, g->draw_fbo);
...
wgl_drawElementsInstanced(JSContext *ctx, JSValueConst this_val, int argc, JSValueConst *argv)
{
    ...
    if (instances < 0 || !wgl_elements_in_range(count, type, offset))
        return JS_UNDEFINED;
...
wgl_getBufferSubData(JSContext *ctx, JSValueConst this_val, int argc, JSValueConst *argv)
{
    ...
    if (dst && len > 0 && wgl_buffer_range_ok(target, offset, len)) {
...
wgl_getInternalformatParameter(JSContext *ctx, JSValueConst this_val, int argc, JSValueConst *argv)
{
    ...
    if (count <= 0 || count > 64) {
        ...
        JSValue ta = JS_NewTypedArray(ctx, 1, &ab, JS_TYPED_ARRAY_INT32);
        JS_FreeValue(ctx, ab);
        return ta;
    }
...
wgl_getActiveUniformBlockParameter(JSContext *ctx, JSValueConst this_val, int argc, JSValueConst *argv)
{
    ...
    if (pname == GL_UNIFORM_BLOCK_ACTIVE_UNIFORM_INDICES) {
        ...
        JSValue ab = JS_NewArrayBufferCopy(ctx, (const uint8_t *)vals,
                                           (size_t)count * sizeof(GLint));
        JSValue ta = JS_NewTypedArray(ctx, 1, &ab, JS_TYPED_ARRAY_UINT32);
        JS_FreeValue(ctx, ab);
        g_free(vals);
        return ta;
    }
...
ns_webgl_canvas_surface(const ns_node *canvas)
{
    ...
    uint8_t *rgba = g_try_malloc((size_t)w * (size_t)h * 4);
    ...
    for (int y = 0; y < h; y++) {
        ...
        for (int x = 0; x < w; x++) {
            ...
            unsigned a = g->alpha ? src[x * 4 + 3] : 255u;
