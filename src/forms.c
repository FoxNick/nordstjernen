...
ns_clear_radio_group(ns_node *root, const ns_node *doc, const ns_node *owner,
                     const char *name, const ns_node *keep, int depth)
{
    if (!root || depth >= NS_FORM_MAX_DEPTH) return;
...
ns_clear_radio_group_for(const ns_node *doc, const ns_node *keep)
{
    ...
    ns_clear_radio_group(root, d, owner, name, keep, 0);
}
...
static gboolean
ns_form_has_file_upload_depth(const ns_node *form, const ns_node *n,
                             const ns_node *doc, int depth)
{
    if (!n || depth >= NS_FORM_MAX_DEPTH) return FALSE;
    ...
    for (const ns_node *c = n->first_child; c; c = c->next_sibling)
        if (ns_form_has_file_upload_depth(form, c, doc, depth + 1)) return TRUE;
    return FALSE;
}
...
{
    return ns_form_has_file_upload_depth(form, n, doc, 0);
}
...
static void
ns_form_collect_multipart_depth(const ns_node *form, const ns_node *n,
...
recurse_mp:
    for (const ns_node *c = n->first_child; c; c = c->next_sibling)
        ns_form_collect_multipart_depth(form, c, doc, body, boundary,
                                        submitter, depth + 1);
}
...
ns_form_collect_multipart(const ns_node *form, const ns_node *n,
                       ...
                       const char *boundary, const ns_node *submitter)
{
    ns_form_collect_multipart_depth(form, n, doc, body, boundary, submitter, 0);
}
