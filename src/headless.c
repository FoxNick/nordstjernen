...
static gboolean
headless_attr_true(const nd_node *n, const char *name)
{
...
static gboolean
headless_attr_false(const nd_node *n, const char *name)
{
    const char *v = nd_element_get_attr(n, name);
...
headless_drag_source_at(nd_box *layout, double x, double y)
{
    const nd_box *hit = layout ? nd_box_hit_test(layout, x, y) : NULL;
    for (const nd_node *p = hit ? hit->dom : NULL; p; p = p->parent) {
...
{
    nd_box *layout = fc && fc->layout ? *fc->layout : NULL;
    const nd_box *hit = layout ? nd_box_hit_test(layout, x, y) : NULL;
    if (hit && hit->dom) return hit->dom;
    if (!fc || !fc->doc) return NULL;
    nd_node *body = nd_node_find_first_element(fc->doc, "body");
    return body ? body : fc->doc;
}

static void
headless_seed_drag_data(headless_flush_ctx *fc, nd_js_drag_session *session,
                        const nd_node *source)
{
    ...
    if (!raw || !*raw) return;
    char *abs = fc->base ? nd_url_resolve(fc->base, raw) : g_strdup(raw);
    if (!abs) return;
    nd_js_drag_session_set_data(session, "text/plain", abs);
    nd_js_drag_session_set_data(session, "text/uri-list", abs);
    g_free(abs);
}

static gboolean
headless_dispatch_drag(headless_flush_ctx *fc, nd_js_drag_session *session,
                       const nd_node *target, const char *type,
                       double x, double y, int buttons,
                       const nd_node *related)
{
    if (!fc || !fc->js || !session || !target) return FALSE;
    gboolean prevented = FALSE;
    nd_js_dispatch_drag_event(fc->js, session, target, type,
                              x, y, x, y, 0, buttons,
              ...
              double x0, double y0, double x1, double y1)
{
    ...
    const nd_node *source = headless_drag_source_at(layout, x0, y0);
    if (!source) return;
    nd_js_drag_session *session = nd_js_drag_session_new(fc->js);
    if (!session) return;
    ...
    gboolean start_prevented =
        headless_dispatch_drag(fc, session, source, "dragstart",
                               x0, y0, 1, NULL);
    if (!start_prevented) {
        const nd_node *target = headless_drag_target_at(fc, x1, y1);
    ...
    nd_js_drag_session_free(session);
}
