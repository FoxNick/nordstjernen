...
void ProcView::ensureConsole() {
    if (m_console)
        return;
    m_console = new QWidget(viewport());
    ...
    auto *title = new QLabel(QStringLiteral("Console"), header);
...
void ProcView::layoutConsole() {
    if (!m_console || !m_console->isVisible())
        return;
    const int h = qMin(260, viewport()->height() / 2);
    m_console->setGeometry(0, viewport()->height() - h, viewport()->width(), h);
    m_console->raise();
}
