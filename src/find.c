...
static void
ns_window_update_match_count(ns_window *w)
{
    ...
    guint total = ns_box_count_matches(w->layout_tree, w->search_query,
                                       w->search_case_sensitive);
    guint cur = w->search_active_box
        ? ns_box_match_ordinal(w->layout_tree, w->search_query,
                               w->search_active_box,
                               w->search_case_sensitive)
        : 0;
    char *msg;
...
on_search_changed(GtkEditable *entry, gpointer user_data)
{
    ...
    ns_paint_set_search(w->search_case_sensitive, NULL);
    gtk_widget_queue_draw(w->drawing_area);
    ns_window_update_match_count(w);
}

static void
ns_window_scroll_to_match(ns_window *w, const ns_box *target)
{
    if (!target || !w->render_vadj) return;
    w->search_active_box = target;
    ns_paint_set_search(w->search_case_sensitive, target);
...
static void
ns_window_search_advance(ns_window *w, gboolean backward)
{
    if (!w->search_query || !w->layout_tree || !w->render_vadj) return;
    double current_y = gtk_adjustment_get_value(w->render_vadj);
    const ns_box *target;
    if (backward) {
        target = ns_box_first_match_above(w->layout_tree, w->search_query,
        ...
        target = ns_box_first_match_below(w->layout_tree, w->search_query,
                                          current_y + 2,
                                          w->search_case_sensitive);
        if (!target)
            target = ns_box_first_match_below(w->layout_tree, w->search_query,
    ...
    if (target) ns_window_scroll_to_match(w, target);
}
...
{
    (void)entry;
    ns_window_search_advance(user_data, FALSE);
}
...
void
on_win_find_next(GSimpleAction *action, GVariant *parameter, gpointer user_data)
{
    (void)action; (void)parameter;
    ns_window *w = user_data;
    if (w->search_revealer &&
        !gtk_revealer_get_reveal_child(GTK_REVEALER(w->search_revealer))) {
        on_win_find(NULL, NULL, w);
        return;
    }
    ns_window_search_advance(w, FALSE);
}

void
on_win_find_prev(GSimpleAction *action, GVariant *parameter, gpointer user_data)
{
    (void)action; (void)parameter;
    ns_window *w = user_data;
    if (w->search_revealer &&
    ...
    ns_window_search_advance(w, TRUE);
}
