...
task_mgr_refresh(NsTaskMgr *tm)
{
    ...
    for (int i = 0; i < n; i++) {
        ...
        long rss = -1;
        if (pid > 0) ns_rproc_http_proc_info(pid, state, sizeof state, &rss);
...
act_settings(GSimpleAction *action, GVariant *parameter, gpointer user_data)
{
    ...
    gtk_grid_attach(GTK_GRID(grid), s->search, 1, 1, 1, 1);
    ...
    gtk_grid_attach(GTK_GRID(grid), font_l, 0, 2, 1, 1);
    gtk_grid_attach(GTK_GRID(grid), s->font_size, 1, 2, 1, 1);

    s->images  = settings_add_switch(GTK_GRID(grid), 3, "Load images",
                                     cfg ? cfg->images_enabled : TRUE);
    s->webgl   = settings_add_switch(GTK_GRID(grid), 4, "Enable WebGL",
                                     cfg ? cfg->webgl_enabled : FALSE);
    s->storage = settings_add_switch(GTK_GRID(grid), 5, "Enable local storage",
                                     cfg ? cfg->local_storage_enabled : TRUE);
    s->dnt     = settings_add_switch(GTK_GRID(grid), 6, "Send Do Not Track",
                                     cfg ? cfg->do_not_track : FALSE);
    s->cache   = settings_add_switch(GTK_GRID(grid), 7, "Enable cache",
