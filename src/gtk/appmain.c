...
{
    if (!g_getenv("NS_WEBGL_ALLOW"))
        g_setenv("NS_WEBGL_ALLOW", "1", TRUE);
    ns_net_init();
...
main(int argc, char **argv)
{
    ...
    if (proc_mode) {
         ...
         * the sandboxed renderer processes. */
        ns_security_add_writable_dir("/dev/shm");
        char *rpath = ns_proc_renderer_path();
        if (rpath) {
            char *rdir = g_path_get_dirname(rpath);
            if (rdir) {
                ns_security_add_exec_dir(rdir);
                g_free(rdir);
            }
            g_free(rpath);
        }
    ...
    if (proc_mode) {
        if (ns_watchdog_is_child(argc, argv)) {
            ...
            /* The shell runs no page JS; a 30 s no-heartbeat budget catches a
             * genuinely wedged UI loop (blit/IPC deadlock), not normal work. */
            ns_watchdog_child_arm_hang_monitor(0);
        }
