...
static ns_media_open_uri_fn ns_media_open_uri = NULL;

void
ns_media_set_open_uri_handler(ns_media_open_uri_fn fn)
{
    ns_media_open_uri = fn;
}
...
static char *
ns_media_suggest_message(gboolean is_video, gboolean stream, const char *app)
{
    if (stream)
...
typedef struct {
    GtkWindow *parent;
    char      *app_url;
} ns_media_suggest_ctx;

static void
ns_media_suggest_response(GObject *src, GAsyncResult *res, gpointer ud)
{
    ns_media_suggest_ctx *c = ud;
    int idx = gtk_alert_dialog_choose_finish(GTK_ALERT_DIALOG(src), res, NULL);
    if (idx == 0 && ns_media_open_uri && c->parent)
        ns_media_open_uri(c->parent, c->app_url);
    if (c->parent)
...
static void
ns_media_suggest(GtkWindow *parent, const char *message,
                 const char *app, const char *app_url)
{
    ...
    ns_media_suggest_ctx *c = g_new0(ns_media_suggest_ctx, 1);
    c->parent = parent;
    c->app_url = g_strdup(app_url);
    if (parent)
        g_object_add_weak_pointer(G_OBJECT(parent), (gpointer *)&c->parent);
    gtk_alert_dialog_choose(dlg, parent, NULL, ns_media_suggest_response, c);
...
gboolean
ns_media_launch_external(GtkWindow *parent, const char *url,
                         gboolean is_video, gboolean stream)
{
    if (!url || !*url) return FALSE;

#if defined(G_OS_WIN32)
    if (!ns_media_url_is_safe(url, FALSE)) return FALSE;
    ...
    char *m = ns_media_suggest_message(is_video, stream, "VLC");
    ns_media_suggest(parent, m, "VLC", "https://videolan.org");
    g_free(m);
    return FALSE;
    ...
    for (int i = 0; apps[i]; i++) {
        ...
        if (ns_media_spawnv(argv)) return TRUE;
    }
    char *argv[] = { (char *)"open", (char *)url, NULL };
    if (ns_media_spawnv(argv)) return TRUE;
    char *m = ns_media_suggest_message(is_video, stream, "IINA");
    ns_media_suggest(parent, m, "IINA", "https://iina.io");
    g_free(m);
    return FALSE;
#else
    if (!ns_media_url_is_safe(url, stream ? FALSE : TRUE)) return FALSE;
    if (!ns_media_player_available(url, stream)) {
        char *m = ns_media_suggest_message(is_video, stream, "mpv");
        ns_media_suggest(parent, m, "mpv", "https://mpv.io");
        g_free(m);
        return FALSE;
    }
    if (stream && !ns_media_ytdlp_available()) {
        ns_media_suggest(parent,
...
#if defined(__linux__)
    if (ns_media_broker_send(url)) return TRUE;
#endif
    return ns_media_run_player(url);
#endif
}
