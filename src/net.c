...
static const char k_about_start_template[] =
    ...
    ".wrap { max-width: 920px; margin: 0 auto; padding: 14px 24px 25px 24px;"
    ...
    " animation: ndfade 0.18s ease both; }\n"
    "@keyframes ndfade { from { opacity:0; transform:translateY(5px); }"
    ...
    " animation: ndpulse 1.2s ease-in-out infinite; }\n"
    "@keyframes ndpulse { 0%,100% { opacity:0.45; } 50% { opacity:0.95; } }\n"
    ...
    " flex:0 0 auto; }\n"
    ".search button, form.ask button { transition:background 0.15s ease,"
    ...
    "</form>"
    "<p class=\"footer\"><span id=\"active\"></span>Local inference \xc2\xb7 "
