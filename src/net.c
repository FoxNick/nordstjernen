...
static const char k_about_start_template[] =
    ...
    "</form>"
    "<p class=\"footer\">"
    "<a href=\"#\" id=\"aitoggle\">Disable AI</a> \xc2\xb7 "
    ...
    "var aiOff=false;\n"
    "try{aiOff=localStorage.getItem('ns_ai_disabled')==='1';}catch(e){}\n"
    "var aitoggle=document.getElementById('aitoggle');\n"
    "var aifoot=document.getElementById('aifoot');\n"
    "function applyAi(){\n"
    " if(aitoggle) aitoggle.textContent=aiOff?'Enable AI':'Disable AI';\n"
    " var d=aiOff?'none':'';\n"
    " if(log) log.style.display=d;\n"
    " if(form) form.style.display=aiOff?'none':'flex';\n"
    " if(aifoot) aifoot.style.display=d;}\n"
    "applyAi();\n"
    "if(aitoggle) aitoggle.addEventListener('click',function(e){\n"
    " e.preventDefault(); aiOff=!aiOff;\n"
    " try{localStorage.setItem('ns_ai_disabled',aiOff?'1':'0');}catch(e){}\n"
    " applyAi();\n"
    " if(aiOff){ if(poll){clearInterval(poll); poll=null;} }\n"
    " else { refresh(); }});\n"
